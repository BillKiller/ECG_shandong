...
def save_ckpt(state, is_best, model_save_dir):
    current_w = os.path.join(model_save_dir, config.current_w + config.kfold)
...
def train_epoch(model, optimizer, criterion, train_dataloader, show_interval=10):
    ...
    for inputs, target in tqdm(train_dataloader, desc='Training'):
...
def integrate_test(args):
    ...
    # model
    model_zoo = []
    for model_name in os.listdir('./model_zoo/'):
        if not model_name.endswith('pth'):
            continue
        model = getattr(models, config.model_name)()
        model.load_state_dict(torch.load('model_zoo/' + model_name , map_location='cpu')['state_dict'])
        model.eval()
        model_zoo.append(model)
    
    # model = model.to(device)
    
    sub_file = '%s/subA_integrate_%s.csv' % (config.sub_dir, time.strftime("%Y%m%d%H%M"))
    ...
    with torch.no_grad():
        for model in model_zoo:
            model = model.to(device)
            test_id = pd.read_csv('./label_and_example/topredict_id.csv')['id']
            single_model_output = []
            for id in tqdm(test_id):
                file_path = os.path.join(config.train_dir, id + '.csv' ) 
                df = pd.read_csv(file_path, sep=',').values
                x = transform(df).unsqueeze(0).to(device)
                output = torch.sigmoid(model(x)).squeeze().cpu()
                single_model_output.append(output)
            single_model_output = torch.stack(single_model_output, dim = 0)
            all_model_output.append(single_model_output)
        all_model_output = torch.stack(all_model_output, dim = 0)
        all_model_output = torch.mean(all_model_output, dim =0)
        idx = 0
        for id in tqdm(test_id):
            output = all_model_output[idx].cpu().numpy()
            ixs = []
            for i, out in enumerate(output):
                if out > 0.5:
                    ...
                    ixs.append("0")
            result.append([id, ",".join(ixs)])
            idx += 1
    df = pd.DataFrame(result, columns=['id','label'])
    df.to_csv('{}/subA_{}.csv'.format(config.sub_dir, time.strftime("%Y%m%d%H%M")), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--kfold", type=str, help="fold name", default="")
